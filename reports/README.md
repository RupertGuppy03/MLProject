...
| rf_calibrated | 760 | 1.0274 | 0.6168 |
...
| rf_calibrated | 2024 | 380 | 1.0091 | 0.6031 |
| rf_calibrated | 2025 | 380 | 1.0457 | 0.6304 |

## Calibration

`rf_calibrated` is the tuned Random Forest wrapped in `CalibratedClassifierCV` (method=`sigmoid` / Platt, `cv=TimeSeriesSplit` so the calibrator never sees future data). Compare its Brier score to `rf_tuned` above. Reliability before/after is plotted in `reports/calibration_curve.png` (generated by `python -m src.models.calibrate`).
