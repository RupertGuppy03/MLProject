...
def rf_calibrated_backtest_model() -> BacktestModel:
    """Tuned RF wrapped in time-safe probability calibration (sigmoid + TimeSeriesSplit).

    Same tuned params as rf_tuned, so the only delta is calibration. The inner TimeSeriesSplit
    runs on each fold's training rows only — nested but leakage-safe.
    """
    ...
    def fit(X_train, y_train, _meta_train):
        base = RandomForestClassifier(**(rf_load_best_params() or {}))
        model = CalibratedClassifierCV(
            estimator=base, cv=TimeSeriesSplit(n_splits=3), method="sigmoid"
        )
        return model.fit(X_train, y_train)

    def predict(state, X_test):
        proba = state.predict_proba(X_test)
        classes = list(state.classes_)
        order = [classes.index(label) for label in LABEL_ORDER]
        return proba[:, order]

    return BacktestModel("rf_calibrated", fit, predict)
...
def _render_readme(metrics: pd.DataFrame, predictions: pd.DataFrame) -> str:
    ...
    # Calibration note (only when the calibrated model is part of the run).
    if "rf_calibrated" in set(predictions["model"]):
        lines += [
            "",
            "## Calibration",
            "",
            "`rf_calibrated` is the tuned Random Forest wrapped in `CalibratedClassifierCV` "
            "(method=`sigmoid` / Platt, `cv=TimeSeriesSplit` so the calibrator never sees future "
            "data). Compare its Brier score to `rf_tuned` above. Reliability before/after is "
            "plotted in `reports/calibration_curve.png` (generated by `python -m src.models.calibrate`).",
        ]
...
def main() -> None:
    ...
    if RF_BEST_PARAMS_PATH.exists():
        ...
        models.append(rf_calibrated_backtest_model())
